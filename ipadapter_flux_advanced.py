...
class ApplyIPAdapterFluxChromaAdvanced:
    ...
    def apply_ipadapter_flux_advanced(self, model, ipadapter_flux, image, weight_start, weight_end, steps, start_percent, end_percent, offload_preprocessing=True):
        ...
        try:
            ...
            if additional_models:
                bi.set_additional_models("ipadapter_flux_advanced", additional_models)
        except Exception as e:
            import traceback
            logging.warning(f"IPAdapter-FluxChroma Advanced: Could not register attention processors: {e}")
        ...
        if offload_preprocessing:
            try:
                ...
                if hasattr(ipadapter_flux.image_encoder, 'cpu'):
                    ...
                    offloaded_count += 1
                    logging.info(f"IPAdapter-FluxChroma Advanced: Offloaded SiglipVisionModel from {original_device} to CPU (~1.1GB VRAM freed)")
                if hasattr(ipadapter_flux.image_proj_model, 'cpu'):
                    ...
                    offloaded_count += 1
                    logging.info(f"IPAdapter-FluxChroma Advanced: Offloaded projection model from {original_device} to CPU (~0.1GB VRAM freed)")
                # Clear CUDA cache to immediately reclaim memory
                if offloaded_count > 0 and torch.cuda.is_available():
                    torch.cuda.empty_cache()
            except Exception as e:
                import traceback
                logging.warning(f"IPAdapter-FluxChroma Advanced: Could not offload preprocessing models: {e}")
                logging.debug(traceback.format_exc())
        else:
            ...
            try:
                ...
                if hasattr(ipadapter_flux.image_encoder, 'to'):
                    ...
                    if current_device.type == 'cpu':
                        ...
                        moved_count += 1
                        logging.info(f"IPAdapter-FluxChroma Advanced: Moved SiglipVisionModel from CPU back to {target_device}")
                if hasattr(ipadapter_flux.image_proj_model, 'to'):
                    ...
                    if current_device.type == 'cpu':
                        ...
                        moved_count += 1
                        logging.info(f"IPAdapter-FluxChroma Advanced: Moved projection model from CPU back to {target_device}")
            except Exception as e:
                import traceback
                logging.warning(f"IPAdapter-FluxChroma Advanced: Could not move preprocessing models back to GPU: {e}")
                logging.debug(traceback.format_exc())
